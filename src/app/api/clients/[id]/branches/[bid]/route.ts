...
export async function PATCH(
  ...
  const { id: clientId, bid } = await params;
  ...
  if (isDelegate && role === "viewer") {
    return NextResponse.json(apiError("Viewers cannot edit branches", "FORBIDDEN"), { status: 403 });
  }
  ...
  if (error || !data) return NextResponse.json(apiError("Branch not found", "NOT_FOUND"), { status: 404 });
...
export async function DELETE(
  ...
  if (isDelegate && role === "viewer") {
    return NextResponse.json(apiError("Viewers cannot delete branches", "FORBIDDEN"), { status: 403 });
  }
