...
function buildTheme(theme: InvoiceTheme, accentColor?: string | null) {
  ...
  const accent = resolveAccentColor(accentColor, base.accent);
  if (accent !== base.accent) {
    base.accent = accent;
    base.titleColor = accent;
    base.grandTotalColor = accent;
    base.dividerColor = accent;
    // Minimal theme has a light gray table header — don't override that
    if (theme !== "minimal") {
      base.tableHeaderBg = accent;
    }
  }
  return base;
}
